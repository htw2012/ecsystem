 ...
 * @version 1.0
 */
public class Iselect extends JFrame implements ActionListener {
	...
	JLabel jLab1 = new JLabel("请填写以下查询信息");
	JLabel jLab2 = new JLabel("货品名称");
	JLabel jLab3 = new JLabel("货品类别");
	...
	private void jbInit() throws Exception {
		...
		pan3.setPreferredSize(new Dimension(350, 40));// ````````````
		pan4.setBackground(new Color(175, 210, 250));
		pan4.setPreferredSize(new Dimension(350, 120));
		pan5.setBackground(new Color(175, 210, 250));
		pan5.setPreferredSize(new Dimension(350, 175));
		pan100.setBackground(new Color(175, 210, 250));
		pan100.setPreferredSize(new Dimension(400, 300));
		pan1.setLayout(flow);
		...
		pan100.add(pan5, BorderLayout.CENTER);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jBtn1) {
			if (jText1.getText().equals("")
					&& jCom.getSelectedItem().equals("")
					&& jText3.getText().equals("")
					&& jText4.getText().equals("")
					&& jText5.getText().equals("")
					&& jText6.getText().equals("")
					&& jText7.getText().equals("")
					&& jText8.getText().equals("")) {
			...
			if (!jText1.getText().trim().equals("")) {
				sql += "Gname='" + jText1.getText().trim() + "'";
				isfirst = false;
			}
			if (!jCom.getSelectedItem().equals("")) {
				if (!isfirst) {
					sql += "and Gtype='" + jCom.getSelectedItem() + "'";
				} else {
			...
			if (!jText3.getText().trim().equals("")) {
				if (!isfirst) {
					sql += "and GIyear>="
							+ Integer.parseInt(jText3.getText().trim());
				} else {
					sql += "GIyear>="
							+ Integer.parseInt(jText3.getText().trim());
				}
			...
			if (!jText5.getText().trim().equals("")) {
				if (!isfirst) {
					sql += "and GIday>="
							+ Integer.parseInt(jText5.getText().trim());
				} else {
					sql += "GIday>="
							+ Integer.parseInt(jText5.getText().trim());
				}
			...
			if (!jText6.getText().trim().equals("")) {
				if (!isfirst) {
					sql += "and GIyear<="
							+ Integer.parseInt(jText6.getText().trim());
				} else {
					sql += "GIyear<="
							+ Integer.parseInt(jText6.getText().trim());
				}
			...
			if (!jText7.getText().trim().equals("")) {
				if (!isfirst) {
					sql += "and GImonth<="
							+ Integer.parseInt(jText7.getText().trim());
				} else {
					sql += "GImonth<="
							+ Integer.parseInt(jText7.getText().trim());
				}
			...
			if (!jText8.getText().trim().equals("")) {
				if (!isfirst) {
					sql += "and GIday<="
							+ Integer.parseInt(jText8.getText().trim());
				} else {
					sql += "GIday<="
							+ Integer.parseInt(jText8.getText().trim());
				}
			...
			if (!isexist) {
				JOptionPane.showMessageDialog(null, "查找信息与数据库中信息不符，请确定后再输入");
			} else {
				ImportShow importshow = new ImportShow();

				Dimension frmSize = getSize();
				Dimension dlgSize = importshow.getSize();
				Point loc = getLocation();
				importshow.setLocation((frmSize.width - dlgSize.width) / 2
						+ loc.x, (frmSize.height - dlgSize.height) / 2 + loc.y);

				importshow.showResult(rs);
			}
